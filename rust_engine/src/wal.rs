...
impl WriteAheadLog {
    ...
    pub fn append_entry(&mut self, view: u64, seq: u64, phase_u8: u8, digest: &[u8; 32]) -> std::io::Result<()> {
        ...
        F: FnMut(u64, u64, u8, [u8; 32])
    {
        ...
        while cursor + 41 <= buf.len() {
            ...
            digest.copy_from_slice(&buf[cursor+17..cursor+49]);

            callback(view, seq, phase_u8, digest);
            cursor += 49;
        }
        Ok(())
    }
