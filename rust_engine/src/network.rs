...
impl NetworkNode {
    ...
    pub async fn send_message(&self, target_id: u32, payload: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
        let peer_addr = self.peers.get(&target_id)
            .ok_or("NETWORK_ERROR: Target not found in registry!")?;
        ...
        println!("📤 [NODE {}]: Sent {} bytes to {}", self.node_id, payload.len(), target_id);
        Ok(())
    }
}

/// The clean, module-level function your project expects
        ...
        tokio::spawn(async move {
            ...
            if socket.read_exact(&mut len_buf).await.is_ok() {
                ...
                if socket.read_exact(&mut buffer).await.is_ok() {
                    println!("📥 [NETWORK]: Received {} bytes from {}", len, peer_addr);
                    handler(0, buffer);
                }
            }
        });
